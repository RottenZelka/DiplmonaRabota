...
const EditQuestion: React.FC = () => {
  ...
  useEffect(() => {
    const fetchQuestionData = async () => {
      try {
        ...
        setMaxPoints(question.max_points);
        setQuestionTypes(typesRes.types);

        if (question.question_type === 'MCQ') {
          const questionChoices = question.choices.split(',');
          setChoices(questionChoices);
          if (question.correct_answer) {
            setCorrectAnswers(question.correct_answer.split(','));
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await updateExamQuestion(id!, formattedData);
      ...
      setError('Failed to update question. Please check the console for details.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ maxWidth: 600, mx: 'auto' }}>
        ...
        <Box sx={{ mt: 4, textAlign: 'center' }}>
          <Button type="submit" variant="contained" color="primary" disabled={loading}>
            {loading ? <CircularProgress size={24} /> : 'Update Question'}
          </Button>
        </Box>
