...
const StudentViewingSchool = ({ profile }) => {
  ...
  const { isApplied, appId } = useApplicationStatus(profile.school.user_id, "student");
          ...
          <Grid item xs={12} style={{ textAlign: "center" }}>
            {isApplied === null ? (
              <Button
                variant="contained"
                disabled
                sx={{ backgroundColor: profile.school.secondary_color || "#ccc", color: profile.school.primary_color || "#000" }}
              >
                Loading...
              </Button>
            ) : isApplied ? (
              <Button
                variant="contained"
                sx={{ backgroundColor: profile.school.secondary_color || "#1976d2", color: profile.school.primary_color || "#fff" }}
              ...
              <Button
                variant="contained"
                sx={{ backgroundColor: profile.school.primary_color || "#4caf50", color: profile.school.secondary_color || "#fff" }}
                onClick={handleApply}
              >
